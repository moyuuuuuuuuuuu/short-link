...
class User extends Model
{
    ...
    {
        if (Cache::has('token_' . $this->getOriginal('id'))) {
            return Cache::get('token_' . $this->getOriginal('id'));
        }
        ...
        $data       = [
            ...
            'username' => $this->getOriginal('username'),
        ...
        $key        = random_bytes(32);
        $encrypt    = openssl_encrypt(json_encode($data), 'aes-256-cbc', $key, $iv = random_bytes(16));

        $token = $builder->issuedBy(getenv('JWT_ISSUER','none'))
            ->permittedFor(getenv('JWT_AUDIENCE'))
            ->identifiedBy(md5($this->getOriginal('id')))
            ->issuedAt($now)
            ->expiresAt($now->modify('+' . intval(getenv('JWT_TTL', 1)) . ' hour'))
            ->withClaim('v', $key)
            ->withClaim('iv', $iv)
            ->withClaim('hashData', $encrypt)
            ->getToken($algorithm, $signingKey);
        $token = $token->toString();
        Cache::set('user:JWTtoken:' . $this->getOriginal('id'), $token, intval(getenv('JWT_TTL', 1)) * 3600);
    ...
    public static function noncestr($length = 10)
    {
        // 可以包含在用户名中的字符集
    ...
    public static function createNickName()
    {
        ...
        return $randomXing[rand(0, count($randomXing) - 1)] . $randomMing[rand(0, count($randomMing) - 1)];
    }

    public static function createAvatar($text)
    {
        $text = mb_substr($text, 0, 1);
        $url  = 'https://ui-avatars.com/api/?name=' . $text . '&size=128&background=' . randomHexColor() . '&color=' . randomHexColor() . '&rounded=false';
        return $url;
    }
}
